...
def tile_lattice_to_mask(lattice_tile, mask, tentile_direction, gage_start_px, length_gauge_px, width_ends_px,
                         width_gauge_px):
    ...
    for y in range(gage_start_px, gage_start_px + length_gauge_px - tile_h + 1, tile_h):
        for x in range((width_ends_px - width_gauge_px) // 2, (width_ends_px + width_gauge_px) // 2 - tile_w + 1,
                       tile_w):
...
def main():
    ...
    domain_mask, gage_start_px, length_gauge_px, width_ends_px, width_gauge_px = generate_dogbone_mask(
        ...
        length_gauge_mm=length_gauge_mm, pixel_per_mm=pixel_per_mm)
    tiled_lattice = tile_lattice_to_mask(lattice_tile, domain_mask, tentile_direction, gage_start_px, length_gauge_px,
                                         width_ends_px, width_gauge_px)
